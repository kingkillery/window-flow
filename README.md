...
A lightweight, hybrid window switcher for Windows. AutoHotkey v2 still owns the dashboard, tray menu, and slot configuration, while a small C# helper handles the activation hot path for faster window switching, monitor placement, and optional maximization.
...
3. **Configure Options**:
   - **Saved Checkbox**: If checked, the app remembers the process name (for example `chrome.exe`). If you close the app and reopen it later, Window-Flow will try to find it again.
